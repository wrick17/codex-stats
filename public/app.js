...
function render(data) {
  ...
  const usage=data.weeklyUsage, remaining=Number(usage?.remaining_percent), hasUsage=Number.isFinite(remaining);
  $("kpi-weekly").textContent=hasUsage?`${number.format(remaining)}%`:"—"; $("kpi-weekly-bar").hidden=!hasUsage; $("kpi-weekly-bar").value=hasUsage?remaining:0; $("kpi-weekly-reset").textContent=hasUsage?`remaining · resets ${fmtDate(Number(usage.resets_at)*1000)}`:"OpenAI balance unavailable";
